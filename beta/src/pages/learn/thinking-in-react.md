...
## 从 mockup 开始 {/*start-with-the-mockup*/}

想象一下, 你早已从设计者那儿得到了一个 JSON API 和 mockup。
...
一开始, 在绘制 mockup 中的每个组件和子组件周围绘制盒子并命名它们。如果你与设计师一起工作, 他们可能早已在其设计工具中对这些组件进行了命名。检查一下它们!
...
如果你的 JSON 结构非常棒, 经常会发现其映射到 UI 中的组件结构是一件自然而然的事情。那是因为 UI 和 mockup 常拥有相同的信息结构--即, 相同的形状。将你的 UI 分割到组件, 每个组件匹配到 mockup 中的每个部分。
