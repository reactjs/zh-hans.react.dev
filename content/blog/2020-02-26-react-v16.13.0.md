...
我们今天发布了 React 16.13.0。它包含了 bug 修复和新的不推荐警告，来帮助准备未来的主要版本。

## 新的警告 {#new-warnings}

### Render 期间更新的警告 {#warnings-for-some-updates-during-render}

一个 React component 不应该在 render 期间对其他component造成副作用影响。
...
**这些警告将会帮助你找到应用中由意外的状态改变引起的 bug。** 在极少数情况下，由于渲染，你有意要更改另一个组件的状态，您可以将 `setState` 调用包装为  `useEffect`。
...
我们感谢所有帮助揭示和解决这些和其他问题的贡献者。您可以找到完整的变更日志 [如下](#changelog)。
...
- 在另一个组件的 render 阶段更新 function component 件时发出警告 ([@acdlite](<(https://github.com/acdlite)>) 在 [#17099](https://github.com/facebook/react/pull/17099))
...
- 在 component stacks 中展示更多警告 ([@gaearon](https://github.com/gaearon) 在 [#17922](https://github.com/facebook/react/pull/17922), [#17586](https://github.com/facebook/react/pull/17586))
