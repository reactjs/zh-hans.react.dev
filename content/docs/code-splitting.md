...
此处代码会告知 React，将标签设置为 `'comments'`，并非一个紧急更新。而是一个 [transition](/docs/react-api.html#transitions)，可能需要一些时间。然后 React 会保留旧的 UI 并进行交互，当它准备好时，会切换为 `<Comments />`，具体请参阅 [Transitions](/docs/react-api.html#transitions) 以了解更多相关信息。
