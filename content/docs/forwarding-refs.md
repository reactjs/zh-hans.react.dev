...
React 组件隐藏其实现细节，包括其渲染的输出。其他使用 `FancyButton` 的组件**通常不需要**获取内部的 DOM 元素 `button` 的 [ref](/docs/refs-and-the-dom.html)。这是极好的，因为其防止组件过度依赖其他组件的 DOM 结构。
...
**Ref 转发是一个主动选择加入的特性，其允许某些组件接收 `ref`，并将其向下传递（换句话说，“转发”它）给子组件。**
