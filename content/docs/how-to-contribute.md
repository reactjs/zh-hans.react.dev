...
* 已安装 [Node](https://nodejs.org) LTS、[Yarn](https://yarnpkg.com/en/) v1.2.0+。
* 已安装 [JDK](https://www.oracle.com/technetwork/java/javase/downloads/index.html)。
* 已安装 `gcc` 或不会因为之后需要安装编译器而烦躁，我们的一些依赖可能得经过编译。在 OS X，Xcode 命令行工具已经包含了这些；在 Ubuntu，`apt-get install build-essential` 会安装所需的 package（其它 Linux 发行版的类似命令也有效）；在 Windows 上得做些额外步骤，请参考 [`node-gyp` 安装步骤](https://github.com/nodejs/node-gyp#installation)。
* 熟悉 Git。
