...
将  `setState()`  视为 *请求* 而不是立即更新组件的命令。为了更好地感知性能，React 可能会延迟它，然后再一次更新多个组件。在你需要强制同步应用 DOM 更新的极端情况下，你可以将它们包在 [`flushSync`](/docs/react-dom.html#flushsync) 里，但是这可能会损害性能。
