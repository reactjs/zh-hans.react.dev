...
> 注意：
>
> 从 React 17 开始，React 会自动修改 console 的方法，例如 `console.log()`，以在对生命周期函数的第二次调用中静默日志。然而，在某些 [可以使用替代解决方案](https://github.com/facebook/react/issues/20090#issuecomment-715927125) 的情况下，这可能会导致一些不被期望的行为的发生。

> 从 React 18 开始，React 不会去掉任何日志。不过，如果你安装了 React DevTools，第二次调用的日志则会显得有些暗淡。React DevTools 还提供了一个设置（默认关闭）来完全去掉它们。
