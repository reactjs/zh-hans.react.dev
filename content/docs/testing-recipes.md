...
> 注意：
>
> 此页面假设你正在使用 [Jest](https://jestjs.io/) 作为测试运行器。如果你使用不同的测试运行器，你可能需要调整 API，但整体的解决方案是相同的。在[测试环境](/docs/testing-environments.html)页面阅读更多关于设置测试环境的细节。
...
你可以使用不同的测试模式，但请记住，_即使测试失败_，我们也要执行清理。否则，测试可能会导致“泄漏”，并且一个测试可能会改变另一个测试的行为。这使得它们难以调试。
...
你可能会发现直接使用 `act()` 有点过于冗长。为了避免一些样板代码，你可以使用 [React 测试库](https://testing-library.com/react)，它的助手是用 `act()` 封装的。
...
通常，你可能希望测试组件对于给定的 prop 渲染是否正确。考虑一个基于 prop 渲染消息的简单组件：
...
有些模块可能在测试环境中不能很好地工作，或者对测试本身不是很重要。用虚拟替换来 mock 这些模块可以使你为代码编写测试变得更容易。
