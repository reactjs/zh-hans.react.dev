...
如果想要将一些交互性添加到现有项目中，不必将其重写为 React。只需将 React 添加到现有技术栈中，就可以在任何位置渲染交互式的 React 组件。

</Intro>

<Note>

**你需要安装 [Node.js](https://nodejs.org/en/) 以进行本地开发。**尽管可以 [在线尝试 React](/learn/installation#try-react) 或使用简单的 HTML 页面，但实际上，大多数用于开发的 JavaScript 工具都需要 Node.js。
...
2. **在框架配置中将 `/some-app` 指定为基本路径**（这里有如何进行配置的例子：[Next.js](https://nextjs.org/docs/api-reference/next.config.js/basepath) 与 [Gatsby](https://www.gatsbyjs.com/docs/how-to/previews-deploys-hosting/path-prefix/)）。
...
假设有一个使用另一种技术（无论是像 Rails 这样的服务器技术，还是像 Backbone 这样的客户端技术）构建的现有页面，并且想要在该页面的某个位置呈现交互式的 React 组件。这是集成 React 的常见方式——实际上，这是多年来大多数 React 用户关注 Meta 的方式！

你可以分两步进行：

1. **设置 JavaScript 环境**，使得可以使用 [JSX 语法](/learn/writing-markup-with-jsx)，使用 [`import`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import) / [`export`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/export) 语法将代码拆分为模块，以及从 [npm](https://www.npmjs.com/) 包注册表中使用包（例如，React）。
2. **在想在页面上看到它们的位置呈现 React 组件**。
...
* **如果你的应用程序已经分成了使用 `import` 语句的文件，请尝试使用你已有的设置。** 检查在你的 JavaScript 代码中编写 `<div />`是否会导致语法错误。如果导致语法错误，你可能需要使用 Babel 转换你的 JavaScript 代码，并启用 Babel React 预设来使用 JSX。
...
请注意，`index.html` 中的原始 HTML 内容被保留，但你自己的 `NavigationBar` React 组件现在出现在 HTML 的 `<nav id="navigation">` 中。阅读 [`createRoot` 用法文档](/reference/react-dom/client/createRoot#rendering-a-page-partially-built-with-react) 以了解如何在现有 HTML 页面中呈现 React 组件。
