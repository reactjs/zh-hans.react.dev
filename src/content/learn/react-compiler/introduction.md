...
然而，在某些情况下，开发者可能需要对记忆化进行更精细的控制。`useMemo` 和 `useCallback` Hooks 可以继续与 React 编译器一起使用，作为一种“逃生通道”，用于控制哪些值会被记忆化。一个常见的用例是，如果记忆化的值被用作 effect 的依赖项，即使其依赖项没有实质性变化，effect 也不会反复触发。
