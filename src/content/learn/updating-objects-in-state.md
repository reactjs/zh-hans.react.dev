...
state `x` 从 `0` 变为 `5`，但是*数字 `0` 本身*并没有发生改变。对内置的原始值如数字、字符串和布尔值进行改变是不可能的。
...
然而，虽然严格来说 React state 中存放的对象是可变的，但你也应该将它们视为不可变的 **如同** 对待数字、布尔值和字符串一样去对待它们。你应该替换它们的值，而不是对它们进行修改。
