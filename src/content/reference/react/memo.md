...
React 通常在其父组件重新渲染时重新渲染一个组件。使用 `memo`，你可以创建一个组件，当其新的 props 与旧的 props 相同时，React 不会重新渲染它。这样的组件被称为**memoized**（记忆化的）组件。
...
如果你的应用像此站点一样，大多数交互是粗略的（例如直接替换页面或整个部分），那么通常不需要使用 memoization（记忆化）。另一方面，如果你的应用更像是绘图编辑器，大多数交互是细粒度的（例如移动图形），那么你可能会发现 memoization 非常有用。
...
在其他情况下将组件包装在 `memo` 中是没有任何好处的。这种做法也没有什么明显的危害，因此一些团队会选择不考虑每种情况，并尽可能使用 `memo`。这种方法的缺点是代码变得不易读。此外，并不是所有的 memoization 都是有效的：一个“总是新的”值足以破坏整个组件的 memoization。
