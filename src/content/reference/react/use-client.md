...
* 当从另一个客户端文件导入 `'use client'` 文件时，该命令无效。这允许你编写可同时在服务器和客户端组件中使用的仅客户端组件。
* `'use client'` 文件中的所有代码以及它直接或间接导入的任何模块都将成为客户端模块图的一部分，并且必须发送到客户端并由客户端执行，以便由客户端渲染至浏览器。为了减少客户端包的大小并充分利用服务器，请尽可能将状态（和 `'use-client'` 命令）移动到组件树的较低位置，并将渲染的服务器组件 [作为客户端组件的子组件]/learn/passing-props- to-a-component#passing-jsx-as-children)。
* 由于 props 是跨服务器-客户端边界进行序列化的，因此请注意这些命令的放置可能会影响发送到客户端的数据量；尽量仅使用必要的数据结构。
...
* 像“use client”这样的指令必须位于文件的最开头，位于任何导入或其他代码之上（指令上方的注释可以）。 它们必须用单引号或双引号编写，而不是反引号。 （“use xyz”指令格式有点类似于“useXyz()”Hook 命名约定，但相似之处纯属巧合。）
