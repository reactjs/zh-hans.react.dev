...
React 是一个用于渲染用户界面（UI）的 JavaScript 库。UI 是由按钮、文本和图像等小单元内容构建而成。React 帮助你把它们组合成可重用、可嵌套的*组件*。从 web 端网站到移动端应用，屏幕上的一切都可以分解成组件。在本章节中，你将学习如何创建、定制和有条件地显示 React 组件。
